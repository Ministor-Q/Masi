  ...
  <body >
   ...
   <div class="fd_content">
	<div class="fd_main">
		...
		<div class="xwgg_title"><a href="CywjAction?attri=资源下载_常用文件&pages=1">常用文件</div>
		<div class="xwgg_title"><a href="CywjAction?attri=资源下载_课件&pages=1">课件</div>
		</div>
		...
		<div class="fd_dl_01">
					...
					<div class="dd_con  dd_con_ie6">
						<div class="fd_news_list_contain">
							<ul class="fd_ul_news">
							<c:forEach items="${page.list}" var="zyxz">
								<li><a href="DownloadAction?zyxzcode=${zyxz.zyxzcode}">${zyxz.zyxzname}</a><i class="i_text"></i>
									<span class="sp_right">${zyxz.zyxzloadtime}</span>
								</li>
								</c:forEach>
							</ul>
						...
						<p>
						<span>
	<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${page.currentPage-1}"><</a>
	<c:choose>
	<c:when test="${ page.totalPage <=7 }">
	<c:forEach var="i" begin="1" end="${page.totalPage}" varStatus="status">
     <a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${i}">${i}</a>  
	</c:forEach>
	...
	<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${page.totalPage}">${ page.totalPage }</a>
	</c:when>
	<c:when test="${  page.totalPage >7 && page.currentPage >4 && page.currentPage <= (page.totalPage-4)}">
	<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=1">1</a>
	...
	<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${page.currentPage+1}">></a>
			...
			第一页 上一页
　　　　</c:when> <c:otherwise>
				<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=1">第一页</a>
				<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${page.currentPage-1}">上一页</a></c:otherwise>
			</c:choose>
			<c:choose>
			 <c:when test="${page.currentPage!= page.totalPage}">
				<a
					href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${page.currentPage+1}">下一页</a>&nbsp;
				<a href="CywjAction?attri=资源下载_${sessionScope.attri}&pages=${page.totalPage}">最后一页</a>
			</c:when> <c:otherwise>
　　　　　　下一页 最后一页
　　　　</c:otherwise>
</c:choose>
