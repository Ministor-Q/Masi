 ...
 <link rel="stylesheet" type="text/css" href="" media="all"  id="changeSkin" />
  ...
  <body onload ="getXyjjArticle()">
   ...
   <div class="fd_content">
		...
		<div class="fd_dl_01">
					...
					<div class="dd_con  dd_con_ie6">
						<div class="fd_news_list_contain">
							<ul class="fd_ul_news">
							<c:forEach items="${page.list}" var="djgz">
								<li><a href="OpenArticleAction?code=${djgz.djgzcode}">${djgz.djgzname}</a><i class="i_text">${djgz.djgzreadednum}</i>
									<span class="sp_right">${djgz.djgzloadtime}</span>
								</li>
						...
						<p>
						<span>
	<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${page.currentPage-1}"><</a>
	<c:choose>
	<c:when test="${ page.totalPage <=7 }">
	<c:forEach var="i" begin="1" end="${page.totalPage}" varStatus="status">
     <a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${i}">${i}</a>  
	</c:forEach>
	...
	<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${page.totalPage}">${ page.totalPage }</a>
	</c:when>
	<c:when test="${  page.totalPage >7 && page.currentPage >4 && page.currentPage <= (page.totalPage-4)}">
	<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=1">1</a>
	...
	<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${page.currentPage+1}">></a>
			...
			第一页 上一页
　　　　</c:when> <c:otherwise>
				<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=1">第一页</a>
				<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${page.currentPage-1}">上一页</a>
			</c:otherwise>
			</c:choose>
			<c:choose>
			 <c:when test="${page.currentPage!= page.totalPage}">
				<a
					href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${page.currentPage+1}">下一页</a>&nbsp;
				<a href="DjgzAction?attri=党建工作_${sessionScope.attri}&pages=${page.totalPage}">最后一页</a>
			</c:when> <c:otherwise>
　　　　　　下一页 最后一页
　　　　</c:otherwise>
</c:choose>
