  ...
  <body onload ="getXyjjArticle()">
   ...
   <div class="fd_content">
	<div class="fd_main">
		...
		<div class="xwgg_title"><a href="KygzAction?attri=科研工作_学术交流&pages=1" >学术交流</a></div>
		<div class="xwgg_title"><a href="KygzAction?attri=科研工作_科研成果&pages=1">科研成果</a></div>
		<div class="xwgg_title"><a href="KygzAction?attri=科研工作_研究所&pages=1">研究所</a></div>
		</div>
		...
		<div class="fd_dl_01">
					...
					<div class="dd_con  dd_con_ie6">
						<div class="fd_news_list_contain">
							<ul class="fd_ul_news">
							<c:forEach items="${page.list}" var="kygz">
								<li><a href="OpenArticleAction?code=${kygz.kygzcode}">${kygz.kygzname}</a><i class="i_text">&nbsp;&nbsp;(${kygz.kygzreadednum})</i>
									<span class="sp_right">${kygz.kygzloadtime}</span>
								</li>
								</c:forEach>
							</ul>
						...
						<p>
						<span>
	<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${page.currentPage-1}"><</a>
	<c:choose>
	<c:when test="${ page.totalPage <=7 }">
	<c:forEach var="i" begin="1" end="${page.totalPage}" varStatus="status">
     <a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${i}">${i}</a>  
	</c:forEach>
	...
	<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${page.totalPage}">${ page.totalPage }</a>
	</c:when>
	<c:when test="${  page.totalPage >7 && page.currentPage >4 && page.currentPage <= (page.totalPage-4)}">
	<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=1">1</a>
	...
	<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${page.currentPage+1}">></a>
			...
			第一页 上一页
　　　　</c:when> <c:otherwise>
				<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=1">第一页</a>
				<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${page.currentPage-1}">上一页</a>
			</c:otherwise>
			</c:choose>
			<c:choose>
			 <c:when test="${page.currentPage!= page.totalPage}">
				<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${page.currentPage+1}">下一页</a>&nbsp;
				<a href="KygzAction?attri=科研工作_${sessionScope.attri}&pages=${page.totalPage}">最后一页</a>
			</c:when> <c:otherwise>
　　　　　　下一页 最后一页
　　　　</c:otherwise>
</c:choose>
