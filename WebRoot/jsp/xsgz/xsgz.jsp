   ...
   <div class="fd_content">
	<div class="fd_main">
		...
		<div class="fd_layout_row fd_rcpy" style ="width:15%;height:400px;float: left">
		 <div class="xwgg_title"><a href="XsgzAction?attri=学生工作_团委学生会&pages=1">团委学生会</a></div>
		<div class="xwgg_title"><a href="XsgzAction?attri=学生工作_班级风采&pages=1">班级风采</a></div>
		<div class="xwgg_title"><a href="XsgzAction?attri=学生工作_考研就业&pages=1">考研就业</a></div>
		<div class="xwgg_title"><a href="XsgzAction?attri=学生工作_学生获奖&pages=1">学生获奖</a></div>
		<div class="xwgg_title"><a href="XsgzAction?attri=学生工作_院刊在线&pages=1">院刊在线</a></div>
		</div>
		...
		<div class="fd_dl_01">
					...
					<div class="dd_con  dd_con_ie6">
						<div class="fd_news_list_contain">
							<ul class="fd_ul_news">
							<c:forEach items="${page.list}" var="xsgz">
								<li><a href ="OpenArticleAction?code=${xsgz.xsgzcode}">${xsgz.xsgzname}</a><i class="i_text">&nbsp;&nbsp;(${xsgz.xsgzreadednum})</i>
									<span class="sp_right">${xsgz.xsgzloadtime}</span>
								</li>
						...
						<p>
						<span>
	<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${page.currentPage-1}"><</a>
	<c:choose>
	<c:when test="${ page.totalPage <=7 }">
	<c:forEach var="i" begin="1" end="${page.totalPage}" varStatus="status">
     <a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${i}">${i}</a>  
	</c:forEach>
	...
	<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${page.totalPage}">${ page.totalPage }</a>
	</c:when>
	<c:when test="${  page.totalPage >7 && page.currentPage >4 && page.currentPage <= (page.totalPage-4)}">
	<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=1">1</a>
	...
	<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${page.currentPage+1}">></a>
			...
			第一页 上一页
　　　　</c:when> <c:otherwise>
				<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=1">第一页</a>
				<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${page.currentPage-1}">上一页</a>
			</c:otherwise>
			</c:choose>
			<c:choose>
			 <c:when test="${page.currentPage!= page.totalPage}">
				<a
					href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${page.currentPage+1}">下一页</a>&nbsp;
				<a href="XsgzAction?attri=学生工作_${sessionScope.attri}&pages=${page.totalPage}">最后一页</a>
			</c:when> <c:otherwise>
　　　　　　下一页 最后一页
　　　　</c:otherwise>
</c:choose>
